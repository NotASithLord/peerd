...
1. The extension opens `https://id.peerd.ai/#op=<enroll|assert>` in a tab it
...
3. The extension `postMessage`s a **ceremony request** to the tab, carrying a
   fresh per-ceremony `challenge` and a `nonce`. The page records the exact
   `event.origin` of that request (a `chrome-extension://` /
   `moz-extension://` origin) and will reply to **that origin only**.
...
The extension binds the reply to its request by the `nonce` and the exact
tab id, and verifies the assertion itself against the person-root-signed
