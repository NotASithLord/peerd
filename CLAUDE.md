...
5. **`peerd-runtime`** — agent loop, tool dispatcher, and the tool
   inventory: **59 `BUILTIN_TOOLS`** (inspect, DOM/page, tab, VM,
   Notebook, App, `edit_file`, `spawn_subagent`, do/get/check,
   memory, `request_review`, `read_pdf`, **dweb share/discover/install +
   sovereign controls**) **+ 2 clock + 5 web + `load_skill` = 67 registered**, of
   which **47 reach the main agent on the store build** — the 13
   low-level DOM/page tools (incl. `read_pdf` and `submit_form`) are runner-only
   (`tools/exposure.js`): the main agent reaches the page through
   `do`/`get`/`check`, which dispatch to a disposable browser-runner
   subagent. The 7 dweb tools are exposure-gated to the preview channel
   (`filterByDwebEnabled` — invisible to the agent where `DWEB_ENABLED`
   is false), so 54 reach it there. Plus sessions,
   clock (temporal grounding), subagent orchestrator, voice (Moonshine
   WASM + Web Speech fallback).
...
- Sandboxes — four execution kinds (DESIGN.md §8.5). Three run in their
  ...
  compute (code mode).
- Six-gate dispatcher with full lineage attached to every tool result
  (`persona → exposure → origin → confirmation → egress → audit`).
  Gate posture, honestly: **persona is REAL** — Plan/Act enforcement
  via `decideAction` (Plan refuses side-effecting tools). **exposure
  is REAL** — runner-only tools are refused to the main agent at
  dispatch (`ctx.exposure === 'main'`). **origin** checks the
  denylist (seed + user overlay from the denylist editor).
  **confirmation** is async and policy-driven. **egress** is a
  deliberate no-op IN THE CHAIN — the real teeth live in the
  egress-allowlist pre-tool-use hook and `safeFetch`. **audit**
  records everything (capped retention: 20k entries, pruned
  oldest-first). The trust-mode axis (Open/Scoped/Paranoid) was
...
- Dual distribution: store (no dweb) + preview channels, generated
  ...
  artifact matrix). See `PACKAGING.md`.
