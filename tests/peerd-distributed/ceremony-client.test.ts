...
const reply = (request: any, over: Record<string, any> = {}) => {
  const supplied = Object.hasOwn(over, 'result') ? over.result : assertResult();
  return {
    origin: CEREMONY_ORIGIN,
    source: ceremonyWindow,
    data: {
      peerd: 'ceremony-result', nonce: request.nonce, ok: true, ...over,
      result: supplied && { purpose: request.purpose, ...supplied },
    },
  };
};
...
describe('ceremony request', () => {
  test('binds the canonical RP and a fresh nonce per request', () => {
    ...
    expect(a.purpose).toBe('find-my-devices');
  });
...
describe('ceremony reply acceptance', () => {
  ...
  test('a page-reported failure surfaces as a named refusal, not a result', () => {
    ...
    expect(verdict.defect).toBe('cancelled');
  });
