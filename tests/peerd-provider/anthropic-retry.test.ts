...
describe('callAnthropic — retryable status set', () => {
  test.each([429, 529, 500, 503])('retries %i and recovers on the next attempt', async (status) => {
    ...
    const events = await drain(callAnthropic(baseArgs({ modelEgress: makeModelEgress({ openInference }) }) as any));
    expect(calls).toBe(2);
  ...
  test.each([400, 401, 403, 404])('throws immediately on non-retryable %i', async (status) => {
    ...
    let thrown: any;
    try { await drain(callAnthropic(baseArgs({ modelEgress: makeModelEgress({ openInference }) }) as any)); }
    catch (e) { thrown = e; }
    expect(calls).toBe(1);
  ...
  test('persistent 500 exhausts retries then surfaces ProviderHttpError', async () => {
    ...
    const events: any[] = [];
    try {
      for await (const ev of callAnthropic(baseArgs({ modelEgress: makeModelEgress({ openInference }) }) as any)) events.push(ev);
    } catch (e) { thrown = e; }
