...
import { spawnSubagentTool } from '../../extension/peerd-runtime/tools/defs/spawn-subagent.js';

// The THIN spawn_subagent tool wrapper: it resolves the caller's lineage,
...
// like the async path (async-subagents.js), so a prompt injection the child
...
describe('spawn_subagent tool — argument + lineage guards', () => {
  test('an empty task is refused before any spawn', async () => {
    const r = await spawnSubagentTool.execute({ task: '  ' }, baseCtx() as any);
    expect(r).toEqual({ ok: false, error: 'task_required' });
  });

  test('a missing parent session fails closed', async () => {
    const r = await spawnSubagentTool.execute({ task: 'go' }, baseCtx({ session: {} }) as any);
  ...
  test('async is the default and its content passes straight through (already fenced upstream)', async () => {
    // the async orchestrator (async-subagents.js) fences on reintegration, so the
    // tool must NOT double-wrap — it forwards res.content verbatim.
    let sawReq: any;
    const ctx = baseCtx({
      spawnSubagentAsync: async (req: any) => { sawReq = req; return { ok: true, content: 'started task xyz' }; },
    });
    const r = await spawnSubagentTool.execute({ task: 'research X' }, ctx as any);
...
describe('spawn_subagent tool — MED-1: the SYNC result is fenced as untrusted', () => {
  ...
  test('a normal sync result wraps the body in the untrusted-content fence', async () => {
    const r = await spawnSubagentTool.execute({ task: 'compare', sync: true }, syncCtx({}) as any) as any;
    expect(r.ok).toBe(true);
    expect(r.content).toContain('<untrusted_web_content');
    expect(r.content).toContain('tool="spawn_subagent"');
    expect(r.content).toContain('the answer');
    expect(r.content).toContain('</untrusted_web_content>');
    // the one-line framing stays OUTSIDE the fence (it's trusted metadata)
    expect(r.content).toMatch(/^subagent \(session sub-1, depth 1\) — 2 tool calls, 42ms/);
    const fenceAt = r.content.indexOf('<untrusted_web_content');
    expect(r.content.indexOf('subagent (session')).toBeLessThan(fenceAt);
  });
  ...
  test('a hostile child result cannot break out of the fence (structural defang)', async () => {
    ...
    const r = await spawnSubagentTool.execute({ task: 'read page', sync: true }, syncCtx({ result: attack }) as any) as any;
  ...
  test('a partial (timed-out) sync result flags outside the fence and still wraps the body', async () => {
    const r = await spawnSubagentTool.execute({ task: 'x', sync: true }, syncCtx({ timedOut: true }) as any) as any;
  ...
  test('a refusal is surfaced as an error, not a fenced result', async () => {
    const ctx = baseCtx({
      spawnSubagent: async () => ({ result: 'max depth 5 exceeded', sessionId: null, toolCalls: 0, durationMs: 0, depth: 6, refused: true }),
    });
    const r = await spawnSubagentTool.execute({ task: 'deep', sync: true }, ctx as any) as any;
