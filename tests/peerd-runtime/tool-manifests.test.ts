...
//   - presets-as-data invariants (runner internals stay inside any preset
//     that grants do/get/check — without them the runner spawns tool-less);
...
describe('normalizeToolManifest', () => {
  ...
  test('present-but-garbage fails CLOSED to the empty manifest, never null', () => {
    ...
    expect(normalizeToolManifest(['do'])).toEqual({ allow: [] });
    expect(normalizeToolManifest({ preset: 7, allow: 'do' })).toEqual({ allow: [] });
  });
  ...
  test('keeps a trimmed preset and string-filtered allow', () => {
    ...
    expect(normalizeToolManifest({ allow: ['do', 7, '', 'get'] as any })).toEqual({ allow: ['do', 'get'] });
...
describe('resolveManifestAllow', () => {
  ...
  test('preset resolves to its allow list', () => {
    ...
    expect(allow.has('get')).toBe(true);
    expect(allow.has('do')).toBe(false);
  ...
  test('unknown preset contributes NOTHING — fail-closed, never the full registry', () => {
    ...
    const allow = resolveManifestAllow({ preset: 'no-such-preset', allow: ['get'] })!;
    expect([...allow]).toEqual(['get']);
...
describe('filterDescriptorsByManifest', () => {
  const descs = [{ name: 'do' }, { name: 'vm_boot' }, { name: 'get' }];
  ...
  test('null allow passes everything through; a Set intersects, order preserved', () => {
    ...
    expect(filterDescriptorsByManifest(descs, new Set(['get', 'do'])).map((t) => t.name))
      .toEqual(['do', 'get']);
    expect(filterDescriptorsByManifest(descs, new Set())).toEqual([]);
  });

  test('composes after mainAgentDescriptors — the manifest can NEVER re-expose a runner-only tool to main', () => {
    const all = [{ name: 'do' }, { name: 'click' }, { name: 'navigate' }, { name: 'get' }];
    // a manifest naming the hidden tools (browse-only names navigate for
    // the runner) still yields a main list without them
    const allow = new Set(['do', 'click', 'navigate', 'get']);
...
describe('exposureGate — per-session manifest refusal at dispatch', () => {
  ...
  test('the runner-only main check still wins even when the manifest allows the name', () => {
    // research includes 'click' so the RUNNER may use it; the main agent
  ...
  test('an EMPTY allow-set (fail-closed resolution) refuses everything', () => {
    expect(eg({ name: 'get' }, {}, { toolAllow: new Set() }).allowed).toBe(false);
  });
});
...
describe('session store — toolManifest', () => {
  ...
  test('setToolManifest sets, replaces, and CLEARS (key removed, not emptied)', async () => {
    ...
    const replaced: Session = await store.setToolManifest(s.sessionId, { allow: ['get'] });
    expect(replaced.toolManifest).toEqual({ allow: ['get'] });
