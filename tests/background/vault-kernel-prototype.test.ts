...
describe('minimal vault authority-kernel prototype', () => {
  ...
  test('entry keeps tiny local projections cheaper than their bridge and excludes controller work', async () => {
    ...
    const senderPolicy = readFileSync(
      join(import.meta.dir, '../../extension/background/kernel-sender-policy.js'), 'utf8',
    );
    for (const route of [
      'vault/prfStatus',
    ...
    expect(source).not.toContain('makeKernelAppEditorRoutes');
    expect(source).not.toContain('createKernelSiteClientRoutes');
    expect(source).not.toContain('makeKernelOpfsPostureRoute');
    expect(source).not.toContain('makeKernelVmMetaRoute');
    expect(source).not.toContain('makeKernelVoiceAuditRoute');
    ...
    expect(source).not.toContain("import('./kernel-semantic-runtime.js')");
    expect(source).toContain('makeKernelDemandRoutes');
    expect(source).toContain('KERNEL_DWEB_ROUTE_NAMES.map((name) => [name, disabledDwebRoute])');
    expect(source).toContain("ok: false, error: 'dweb-disabled', outcomeKnown: true");
    ...
    expect(source).not.toContain('const delivered = { ...state, pendingConfirm }');
    expect(source).toContain("uiPorts.broadcast({ type: 'confirm/request', prompt: pendingConfirm })");
    expect(source).not.toContain('globalThis.chrome');
    expect(source).toContain('createKernelSenderPolicy');
    expect(senderPolicy).toContain('isSidepanelSender');
    expect(senderPolicy).toContain('isSidepanelPortSender');
