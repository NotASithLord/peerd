...
// The settings view of the learned origin set, and the only un-learn path. What
// these pin: the origin is CANONICALIZED with the same normalizer `note` uses (a
...
describe('learned/list', () => {
  test('returns the sorted set', async () => {
    const { deps } = makeDeps({ 'https://b.test': 'password-field', 'https://a.test': 'confirmed-write' });
...
describe('learned/forget', () => {
  test('canonicalizes the origin before deleting', async () => {
    // The UI renders `https://app.test`; a caller passing a URL with a path (or a
    // trailing slash) must still hit the same key the classifier looks up.
    const { deps, map } = makeDeps({ 'https://app.test': 'password-field' });
    const r = await makeLearnedOriginRoutes(deps)['learned/forget']({ origin: 'https://app.test/login?x=1' });
    expect(r.ok).toBe(true);
    expect(map.has('https://app.test')).toBe(false);
  });

  test('refuses a non-origin instead of no-oping silently', async () => {
    const { deps } = makeDeps({ 'https://app.test': 'password-field' });
    for (const bad of ['', 'not a url', null, undefined, 42]) {
      expect(await makeLearnedOriginRoutes(deps)['learned/forget']({ origin: bad }))
  ...
  test('reports not-learned for an origin that was never in the set', async () => {
    const { deps } = makeDeps({ 'https://app.test': 'password-field' });
    expect(await makeLearnedOriginRoutes(deps)['learned/forget']({ origin: 'https://other.test' }))
  ...
  test('waits for the durable write before replying', async () => {
    ...
    await makeLearnedOriginRoutes(deps)['learned/forget']({ origin: 'https://app.test' });
...
describe('learned/clear', () => {
  test('empties the set and reports how many it forgot', async () => {
    const { deps, map } = makeDeps({ 'https://a.test': 'password-field', 'https://b.test': 'confirmed-write' });
...
// profile full of learned origins and `learned/clear` reported success while
...
describe('routes wait for the durable set', () => {
  test('learned/list does not report an empty list while the boot read is in flight', async () => {
    ...
    release(null);
    expect(await pending).toEqual({ ok: true, origins: [{ origin: 'https://a.test', reason: 'password-field' }] });
  });
  ...
  test('learned/forget on a cold worker removes the row instead of reporting not-learned', async () => {
    ...
    const pending = routesFor(store)['learned/forget']({ origin: 'https://a.test' });
    release(null);
    expect((await pending).ok).toBe(true);
    await booting;
    expect(store.snapshot().has('https://a.test')).toBe(false);
  });
