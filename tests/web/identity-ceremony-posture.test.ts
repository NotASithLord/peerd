...
describe('ceremony transport is origin-bound and one-shot (the #360 fixes)', () => {
  ...
  test('requests are accepted ONLY from the opener and a pinned Peerd release origin', () => {
    expect(js).toContain('TRUSTED_EXTENSION_ORIGINS = new Set');
    expect(js).toContain('chrome-extension://lpdkhfeldihoejbbfonnbekpjclkknoc');
    expect(js).toMatch(/if\s*\(!TRUSTED_EXTENSION_ORIGINS\.has\(event\.origin\)\)\s*return/);
    expect(js).not.toMatch(/\^\(chrome-extension\|moz-extension/);
