...
/**
 * @param {{ now?: () => number, persist?: { load: () => Promise<any>, save: (s: any) => Promise<void> } | null }} [opts]
 */
export const createDhtStore = ({ now = Date.now, persist = null } = {}) => {
  /** @typedef {import('./records.js').Item} Item */
  // why `item: any` here: a stored item is wire-decoded JSON whose authenticity
  // (not its TS shape) is what put()/verifyItem enforce; callers read its opaque
  // `value` directly, so a strict shape would only force casts at every read.
  /** @type {Map<string, { item: any, storedAt: number }>} */
  ...
  const load = async () => {
    if (loaded) return;
    loaded = true;
    // why guard: when persist is null, `loaded` starts true and we never
    // reach here — the check is redundant at runtime but lets TS narrow.
    if (!persist) return;
    try {
      const raw = await persist.load();
      if (raw && typeof raw === 'object') {
        for (const [k, v] of Object.entries(raw)) items.set(k, /** @type {{ item: any, storedAt: number }} */ (v));
      }
    } catch { /* fresh */ }
  };
  ...
  return {
     ...
     * @param {unknown} item — a wire-decoded DHT item (validated below)
     */
    async put(item) {
      await load();
    ...
    has: (hexKey) => {
      const v = items.get(hexKey);
      return Boolean(v) && now() - /** @type {{ storedAt: number }} */ (v).storedAt <= ITEM_TTL_MS;
    },
