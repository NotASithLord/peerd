...
// "this device key is authorized to act as a device belonging to this
// person did": no capabilities, no scopes.
...
export const rosterSupersedes = (candidate, held) => {
  if (!held) return true;
  if (candidate.personDid !== held.personDid || candidate.seq <= held.seq) return false;
  // Snapshot updates may add devices and revoke active devices, but they may
  // not erase history, rewrite a device id, or resurrect a tombstone. This
  // makes revocation monotone even when a caller accidentally constructs a
  // partial snapshot.
  for (const previous of held.devices) {
    const next = candidate.devices.find((entry) => entry.deviceDid === previous.deviceDid);
    if (!next) return false;
    if (next.deviceId !== previous.deviceId || next.addedAt !== previous.addedAt) return false;
