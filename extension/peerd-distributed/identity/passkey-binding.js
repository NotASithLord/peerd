...
const COMMITMENT_B64_LENGTH = 44; // 32 bytes, padded standard base64
...
export const validatePasskeyBinding = (binding) => {
  ...
  for (const credential of binding.credentials) {
    ...
    // This commitment is derived from a PRF output the sponsor deliberately
    // does not retain. It gives a recovering device an authority independent
    // of the sponsor for deciding which person root this credential belongs
    // to (self/enroll.js verifies it before adopting root material).
    if (typeof credential.identityCommitment !== 'string'
        || credential.identityCommitment.length !== COMMITMENT_B64_LENGTH
        || !BASE64_PATTERN.test(credential.identityCommitment)) return 'bad-identity-commitment';
...
export const bindingSupersedes = (candidate, held) => {
  if (!held) return true;
  if (candidate.personDid !== held.personDid || candidate.rpId !== held.rpId
      || candidate.seq <= held.seq) return false;
  for (const previous of held.credentials) {
    const next = candidate.credentials.find((row) => row.credentialId === previous.credentialId);
    if (!next) return false;
    if (next.alg !== previous.alg || next.publicKeySpki !== previous.publicKeySpki
        || next.identityCommitment !== previous.identityCommitment
        || next.addedAt !== previous.addedAt) return false;
    if (previous.status === 'revoked' && next.status !== 'revoked') return false;
  }
  return true;
};
