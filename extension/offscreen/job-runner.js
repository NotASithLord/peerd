    ...
    return await new Promise((resolve) => {
      ...
      worker.addEventListener('message', async (ev) => {
        ...
        if (m.type === 'subagent-request') {
          ...
          // caps profile (PR #119: the page_code worker) is the second no-subagent
          // lane, enforced the same way.
          if (a2a || !profile.subagent) {
            worker.postMessage({ type: 'subagent-response', rid: m.rid, error: a2a ? 'subagent spawn is disabled for a2a runs (the dweb actor does not delegate)' : 'subagent capability is disabled for this job' });
            return;
          }
          const a = m.args ?? {};
          try {
