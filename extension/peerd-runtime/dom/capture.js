 ...
 *
 * @param {{ id: number, url?: string, peerdDocumentId?: string, peerdDocumentTimeOrigin?: number }} tab the resolved target tab
...
export const captureSnapshot = async (tab, ctx, { budget = 8000 } = {}) => {
  ...
  const debuggerPool = /** @type {{ getAxTree(tabId: number, expectedDocument: ReturnType<typeof browserDocumentIdentity>): Promise<Parameters<typeof serializeAxTree>[0]> } | undefined} */ (
  ...
  if (typeof debuggerPool?.getAxTree === 'function') {
    ...
    // Password-field learning belongs to resolveTargetTab's bounded live-document
    // probe. Repeating that injection here would add a second renderer wait after
    // the target was already approved and could hang an otherwise complete read.
