...
export const messageResidentTool = {
  ...
  primitive: 'subagent',
  description: [
    'Delegate a GOAL to a RESIDENT. For WEB WORK, address `to:"web"` and delegate',
    ...
    'app_list. Each resident is a GenServer (an OTP process): a registered name,',
    'started on first message, holding that environment\'s tools and doing the work in',
    'its own focused context. message_resident is a cast: you hand off the goal and',
    ...
    'just await it. Nothing to poll either way. A resident is STATEFUL and handles one',
    ...
    'residents run in parallel. Your ONLY path to act on a page or mutate an instance.',
  ].join(' '),
  schema: {
    type: 'object',
    properties: {
      to: {
        type: 'string',
        description: 'Who to address: "web" for general web work (the web actor picks fetch-vs-render — prefer this for web tasks); a specific open page\'s tabId (from list_tabs / open_tab); OR a vm/notebook/app instance id (from vm_list / js_list / app_list). A resident is minted on first message.',
      },
      message: {
        type: 'string',
