...
export const messageResidentTool = {
  ...
  description: [
    ...
    'js_list / app_list). Each resident is a GenServer (an OTP process): a',
    'registered name, started on first message, holding that environment\'s tools',
    'and doing the work — driving the page, running commands, editing files,',
    'building the UI — in its own focused context. message_resident is a cast: you',
    'hand off the goal and don\'t wait; the reply lands as a fenced note on a LATER',
    'turn. When your next step needs a value back ("read the price") it\'s a call —',
    'same later-turn reply, you just await it. Nothing to poll either way. A resident',
    'is STATEFUL and handles one message at a time (its mailbox): reuse the same `to`',
    'for follow-up on the same page/instance (no re-orientation); message a DIFFERENT',
    'tab/instance for independent work — separate residents run in parallel. Your',
    'ONLY path to act on a page or mutate an instance.',
  ].join(' '),
  schema: {
    type: 'object',
    properties: {
