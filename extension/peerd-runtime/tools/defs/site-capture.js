...
export const siteCaptureTool = composeTool("site_capture", {
  execute: async (args, ctx) => {
    ...
    const authority = /** @type {{startOwnedCapture?:()=>Promise<{ok:boolean,origin?:string,tap?:string,error?:string}>,stopOwnedCapture?:()=>Promise<{ok:boolean,origin?:string,digest?:any,error?:string}>}|undefined} */ (
      /** @type {any} */ (ctx).siteClientAuthority);
    if (!authority?.startOwnedCapture || !authority.stopOwnedCapture) {
      return { ok: false, error: 'site_capture_unavailable' };
    }
    if (action === 'start') {
      const started = await authority.startOwnedCapture();
      if (started?.ok !== true) return { ok: false, error: started?.error ?? 'site_capture_failed' };
      return { ok: true, content: `capture started on ${started.origin} (via ${started.tap ?? 'tap'}). Drive the site, then site_capture stop.` };
    }
    const stopped = await authority.stopOwnedCapture();
    if (stopped?.ok !== true) return { ok: false, error: stopped?.error ?? 'site_capture_failed' };
    const tabOrigin = stopped.origin;
    if (typeof tabOrigin !== 'string') return { ok: false, error: 'site_capture_failed' };
    const digest = stopped.digest;
