...
const runAgentTurn = async (/** @type {any} */ { userText, attachments = null, sessionId: targetSessionId = null, synthetic = false, resume = false, activeTabId = null }) => {
  ...
  const skillsBlock = await skillRegistry.describeForPrompt().catch((e) => {
  ...
  const costProviderIsLocal = !!listProviders()
    .find((p) => p.name === costSession?.provider)?.keyless;
  const costTracker = makeTurnCostTracker({
    costOf: (model, usage, overrides) =>
    ...
    onLimitExceeded: ({ sessionId: sid, spent, limitUsd }) => {
      if (uiConnected()) {
        uiPorts.broadcast({
  ...
  try {
    for await (const ev of runUserTurn({
      ...
      enrichTrimSummary: (req) => trimEnricher.queue(/** @type {any} */ (req)),
    ...
    trimEnricher.drain(sessionId)
      .catch((e) => console.warn('[sw] trim enrichment failed', e));
    if (uiConnected()) {
