...
export const makeDwebSelfRoutes = (deps) => {
  ...
  return {
     ...
     */
    'dweb/self-prepare-offer': async ({ surfaces, label } = {}) => {
      if (!(await dwebReady())) return { ok: false, error: 'dweb-disabled' };
      ...
      for (const name of surfaces) {
        ...
        if (!shaper) { unavailable.push(name); continue; }
        let shaped;
        try { shaped = await shaper(); } catch { unavailable.push(name); continue; }
        if (shaped === null || shaped === undefined) { unavailable.push(name); continue; }
      ...
      if (entries.length === 0) return { ok: false, error: 'nothing-to-offer', unavailable };
      ...
      try {
        const reply = await callBaseHost('dweb/base-host/self-offer', { manifest });
