...
/**
 * Keep one local, dismissible failure beside authority-pushed notices.
 * @param {ReadonlyArray<any>|undefined} notices
 * @param {unknown} cause
 */
 ...
 *   Activated only when settleUiEffect marks the returned promise as
 *   fire-and-forget; awaited callers keep their own failure presentation.
 */
export const makeReconciledUiSender = ({
  ...
  const pending = (async () => {
    ...
    try {
      ...
      if (!read) {
        // A rejected effect crossed the authority boundary. Reconcile once,
        // never replay, and preserve the original failure for the caller.
