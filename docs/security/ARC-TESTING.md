...
| Navigate an actor-owned Chrome tab directly to a private target. | Chrome DNR blocks the HTTP request, but the isolated target can still observe a TCP connection/preconnect. The live lane requires proof that the probe ran and records that residual; the browser floor is not an absolute zero-transport or port-oracle boundary. |
...
Chrome's immediate inherited-child request
