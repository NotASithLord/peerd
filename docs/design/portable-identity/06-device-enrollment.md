...
```
 Device B (new)              id.peerd.ai              Device A (sponsor)
     ...
     |------------------ ENROLL_REQ ------------------------>|
     |   credentialId, ECDH key, deviceDid + install id, MAC  |
     ...
     |  persist only the bounded grant; never the root seed  |
```
...
Surfaces apply independently. Single-value surfaces are applied as one store
operation; collection surfaces such as chats and Apps are item-atomic because
their backing stores do not offer one cross-item transaction. If a later item
fails, the applier throws a structured partial result naming the durable count,
and the transfer can never report an empty or full success after those effects.
The surface vocabulary is **closed**: a name outside `SYNC_SURFACES` is
refused, never "applied generically".
...
(`offscreen/dweb-self.js`). It starts independently of the public base network,
because a rootless enrolled device deliberately cannot join that network until
the certificate-backed transport gate below lands. When a public base handle
exists it reuses that handle's signaling URL, but it always joins separate
private rendezvous rooms authenticated with the DEVICE key. An install that is
not yet a member of a person's device set stays INERT, because the rendezvous
topics derive from a discovery secret it does not have. It never sees the
person root.
...
The integrity of this page during both ceremonies is a security boundary, not
merely an availability dependency. Compromised canonical-origin code sees the
PRF output, can derive the enrollment rendezvous/MAC, open an attacker-owned
enrollment exchange, then use the expected second user-verification gesture to
request the fresh assertion for that exchange. The sponsor cannot distinguish
that valid canonical-RP assertion from one requested by honest page code.

Consequently deployment must make the page a minimal immutable artifact with
strict CSP, no third-party script or dynamic fetch, reproducible byte review,
and release monitoring. The extension origin/opener/nonce checks protect its
own reply channel and remain necessary, but they do not downgrade a compromise
of `id.peerd.ai` to denial of service. A design that removes this trust would
need extension-controlled ceremony execution at the RP with page script unable
to interfere, or authenticator-visible transaction authorization beyond the
ordinary WebAuthn assertion used here.
...
1. **The live enrollment controller and rendered front door are not wired.**
   The reducer, protocol primitives, private device mesh, sync hosts, custody,
   and both store relay doors run in the shipped runtime (§7b), but no runtime
   controller yet executes the reducer's ceremony/enrollment actions. The sync
   host can ask a selected proven self device to prepare and target an offer,
   but the vault gate's rendered
   "Create new / Use my existing Peerd" fork is also absent. Enrollment and
   restore therefore have no user-facing entry point and are not an end-to-end
   live feature yet; the production contract stops at tested protocol and host
   components.
...
7. **Public transport device-certificate switchover remains a release gate.**
   Mesh envelopes, Agent Cards, manifests, DHT items, and A2A still sign with
   the person root. The enrolled-device grant therefore does not install a
   public-network root at all. Such a device may use the private self-device
   mesh, but must not start the public base network until those surfaces accept
   certificate-backed device signatures. This avoids solving revocation by
   copying the permanent root to every device.
   The design doc's sequencing is explicit that the wire verifier must accept
