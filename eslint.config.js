...
const CROSS_MODULE_IMPORT = {
  ...
  regex: '(^|/)peerd-(?!runtime/(?:administrative|authority|controller(?:-administrative|-tools|-turn|-turn-semantics)?|kernel-executable|semantic|skills)\\.js$|runtime/contacts/(?:store|aggregate)\\.js$|runtime/observability/contributor-store\\.js$|engine/(?:app-assets|artifact|app-manifest|authority)\\.js$|egress/(?:kernel-storage\\.js|confirm/protocol\\.js)$)[a-z]+/(?!index\\.js$|background\\.js$|offscreen\\.js$|repository\\.js$|kernel(?:-credentials|-memory|-network)?\\.js$|ui\\.js$).+',
