...
export default [
  ...
  // --- heap-split reasoning Worker: a WORKER-SAFE subset, not the barrel ---
  // offscreen/reasoning-worker.js is a dedicated Worker (its own heap) that runs
  // the agent loop for a pure-reasoning subagent. It must import a MINIMAL,
  // worker-safe subset (agent-loop.js + reasoning-worker-core.js — both verified
  // to touch no chrome.*/DOM at import) rather than the full /peerd-runtime
  // barrel, which re-exports voice/tools/etc. and would drag chrome-touching
