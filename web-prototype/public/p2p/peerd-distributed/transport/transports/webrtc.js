  ...
  return {
    ...
    async connect(peer, { signaling, sameMachine = false, iceServers: ice } = {}) {
      ...
      const p = createPeer({
        ...
        onCandidate: (c) => signaling.send({ ice: c }),
      });
      const off = signaling.onRemote(async (msg) => {
        if (!msg) return;
      ...
      signaling.send({ type: 'offer', sdp: p.pc.localDescription.sdp });
    ...
    async accept({ offer, signaling, sameMachine = false, iceServers: ice } = {}) {
      ...
      signaling.send({ type: 'answer', sdp: p.pc.localDescription.sdp });
