...
export const joinRoom = async ({
  ...
  const attachSession = (s) => {
    ...
    s.on('signal', async ({ from, payload }) => {
      ...
      try {
        ...
        audit?.('room_accept_failed', { member: from, error: e.message });
      } finally {
        routers.delete(from);
      }
    });
    ...
    const dial = async (member) => {
      ...
      try {
        ...
        audit?.('room_dial_failed', { member, error: e.message });
      } finally {
  ...
  mesh.onRelay(async ({ env, via }) => {
    ...
    if (kind === 'offer') {
      ...
      try {
        ...
        audit?.('relay_accept_failed', { from: env.from, error: e.message });
      } finally {
  ...
  const expandViaPeer = async (viaDid) => {
    const members = await mesh.requestRoster(viaDid);
  ...
  const connectRendezvous = async () => {
    if (left) return;
    const s = await openRendezvous({ url, room: roomId, WebSocket: WS, kind: peerKind });
