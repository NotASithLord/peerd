...
/**
 * @param {{
 *   identity: { did: string, sign: Function },
 ...
 *   audit?: ((t: string, d?: any) => void) | null,
 *   now?: () => number,
 * }} opts
 */
export const createPeerNode = async ({ identity, mesh, meta = () => ({}), dial = null, audit = null, now = Date.now }) => {
  ...
  const offGone = mesh.onPeerGone?.(({ did }) => presence.forget(did)) ?? (() => {});
  ...
  const offSeed = mesh.onPeer?.(({ did }) => dht.learn(did)) ?? (() => {}); // new links seed as they form (close() runs offSeed BEFORE detachDht, so no learn races teardown)
