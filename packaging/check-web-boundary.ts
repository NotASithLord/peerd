// check-web-boundary.ts — the honesty gate for the `web` target (the web
// analogue of verify-store-artifact.ts). Runs over the ASSEMBLED web-dist/
// tree and proves it is actually shippable:
//
...
//      on a redeploy).
//   5. SHIPPABILITY — no file exceeds the 25 MiB static-host per-file limit.
...
export const checkWebBoundary = async (): Promise<void> => {
  ...
  const scanModule = (fromRel: string, src: string, lineBase = 0): void => {
    ...
    // checked: those are page-context loads against this origin. Relative/bare
    // worker paths in staged module code are data for OTHER contexts (the App
    // composer emits `new Worker('…')` into opaque-iframe harness code,
    // resolved against a blob origin) — not resolvable here. Comment-stripped
    // so doc examples don't match at all.
  ...
  // Channel posture. The module itself is not staged (web-target.ts drops
  // peerd-distributed from the include list) — assert that too, the same
  // posture guarantee the store build gets from its dweb-prune.
  ...
  // Shippability: static-host per-file limit (Cloudflare Pages' 25 MiB cap).
  for (const rel of allFiles) {
    const size = statSync(join(WEB_DIST, rel)).size;
    if (size > PAGES_FILE_LIMIT) {
      f.violations.push(`${rel} is ${(size / 1048576).toFixed(1)} MiB — over the 25 MiB static-host per-file limit; a deploy WILL fail`);
    } else if (size > PAGES_FILE_WARN) {
      f.warnings.push(`${rel} is ${(size / 1048576).toFixed(1)} MiB — within ${((PAGES_FILE_LIMIT - size) / 1048576).toFixed(1)} MiB of the 25 MiB limit`);
    }
  }
