...
// 769 → 775: coverage reached 100% of the non-ES5 set. Two files were holding
// it at 99.7% for unrelated reasons. fetch-tap-injected.js is an ES5 injected
// body that eslint.config.js already exempted and tscheck-coverage.ts did not,
// so it sat in the denominator with nothing to fix (the two lists now have a
// parity test). web/public/sw.js was genuinely unchecked; annotating it turned
// up a real bug, a respondWith(undefined) on a cache miss.
